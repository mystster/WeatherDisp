...
unsigned char JPEGDecoder::pjpeg_need_bytes_callback(unsigned char* pBuf, unsigned char buf_size, unsigned char *pBytes_actually_read, void *pCallback_data)
{
    ...
    //memcpy(pBuf, (g_pInData + g_nInFileOfs), sizeof(int8) * n);
    int a = client->readBytes(pBuf, n);

    // Serial.printf("request data size is %d\nreaded data size is %d\ndata: %02x %02x ... %02x\n", n, a, pBuf[0], pBuf[1], pBuf[n-1]);
